...
To demonstrate \sys's effectiveness, we apply it to the Linux kernel's eBPF subsystem, analyzing 16,000+ commits through our multi-agent lens. Our agents uncover nuanced insights: divergent maintainer-contributor perspectives on stability vs. features, hidden dependencies across subsystems, and evolution patterns validated by kernel experts. The multi-agent analysis reveals how different stakeholders view the same commits differently—maintainers focus on stability while contributors push features—providing quantitative evidence of collaboration dynamics. \sys demonstrates that future kernel development will be AI-augmented, with multi-agent systems serving as cognitive bridges between legacy codebases and modern AI tooling. The framework and dataset will be made available upon acceptance.
\end{abstract}
