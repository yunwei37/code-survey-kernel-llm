...
Using CrewAI~\cite{crewai} and advanced reasoning models like o4-mini~\cite{o4mini}, we instantiated Empirically-Grounded Personas representing actual eBPF contributors, such as Alexei Starovoitov (maintainer), Brendan Gregg (observability), and KP Singh (security). Each agent maintains personalized memory systems containing their complete contribution history, email threads, and design philosophies, enabling authentic perspective-based analysis of over 15,000 commits and 150,000+ emails.
...
% Accuracy can be enhanced through: multiple survey runs (we ran once due to budget, achieving <1\% error), domain-specific fine-tuning or advanced models like Claude 4~\cite{claude4} or Gemini 2.5 Pro~\cite{gemini25}, clearer prompts, and multi-step processes for complex commits.
